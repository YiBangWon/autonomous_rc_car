...
class SlideWindow:
    ...
    def slidewindow(self, img):
        ...
        # not worked 09-28
        # pts_left = np.array([[width/2 - 120, height], [width/2 - 120, height - 90], [width/2 - 180, height - 110], [width/2 - 180, height]], np.int32)
        # cv2.polylines(out_img, [pts_left], False, (0,255,0), 1)
        # pts_right = np.array([[width/2 + 107, height], [width/2 + 107, height - 110], [width/2 + 170, height - 130], [width/2 + 170, height]], np.int32)
        # cv2.polylines(out_img, [pts_right], False, (255,0,0), 1)
        # add rect size
        # pts_left = np.array([[125, 480], [125, 390], [215,390], [215, 480]], np.int32)
        # cv2.polylines(out_img, [pts_left], False, (0,255,0), 1)
        #
        # # pts_right = np.array([[415, 480], [415, 370], [505, 370] , [505, 480]], np.int32)
        # # cv2.polylines(out_img, [pts_right], False, (255,0,0), 1)
        # pts_right = np.array([[365, 480], [365, 370], [455, 370] , [455, 480]], np.int32)
        # cv2.polylines(out_img, [pts_right], False, (255,0,0), 1)
        ...
        # not used
        # good_left_inds = ((nonzerox >= width/2 - 180) & (nonzeroy >= nonzerox * 0.33 + 310) & (nonzerox <= width/2 - 120)).nonzero()[0]
        # good_right_inds = ((nonzerox >= width/2 + 107) & (nonzeroy >= nonzerox * (-0.48) + 564) & (nonzerox <= width/2 + 170)).nonzero()[0]
        # good_left_inds = ((nonzerox >= 125) & (nonzeroy >= 390) & (nonzerox <= 215)).nonzero()[0]
        # # good_right_inds = ((nonzerox >= 415) & (nonzeroy >= 370) & (nonzerox <= 505)).nonzero()[0]
        # good_right_inds = ((nonzerox >= 365) & (nonzeroy >= 370) & (nonzerox <= 455)).nonzero()[0]
        ...
        if line_flag != 3:
            ...
            for i in range(len(good_left_inds)):
                    img = cv2.circle(out_img, (nonzerox[good_left_inds[i]], nonzeroy[good_left_inds[i]]), 1, (0,0,255), -1)
            ...
            for window in range(0, nwindows):
                if line_flag == 1:
                    ...
                    cv2.rectangle(out_img, (win_x_low + int(width * lineWidth), win_y_low), (win_x_high + int(width * lineWidth), win_y_high), (255, 0, 0), 1)
                    ...
                    # 0.165 is the half of the road(0.33)
                        ...
                        # x_location = x_current + int(width * lineHalfWidth)
                        cv2.rectangle(out_img, (x_location, 50) , (x_location + 5, 55),(0,0,255),2) # to check x_location
                    ...
                    cv2.rectangle(out_img, (win_x_low - int(width * lineWidth), win_y_low), (win_x_high - int(width * lineWidth), win_y_high), (0, 255, 0), 1)
                    ...
                    # 0.165 is the half of the road(0.33)
                        x_location = x_current - int(width * 0.175)
                        # x_location = x_current - int(width * lineHalfWidth * 1.2)
                        cv2.rectangle(out_img, (x_location, 50) , (x_location + 5, 55),(0,0,255),2) # to check x_location
